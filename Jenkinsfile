...
node {
    ...
    stage("Build image") {
        tryStep "build", {
            def image = docker.build("build.datapunt.amsterdam.nl:5000/datapunt/signalsexport:${env.BUILD_NUMBER}")
            image.push()
        }
    }
}
...
/*    node {
    ...
    node {
        stage('Push production image') {
            tryStep "image tagging", {
                def image = docker.image("build.datapunt.amsterdam.nl:5000/datapunt/gastransitie:${env.BUILD_NUMBER}")
                image.pull()
